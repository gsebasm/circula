...
const ChatbotInterface: React.FC = () => {
    ...
    async function handleSend(e: FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (formState !== 'idle' || input.length <= 0 || input.length !== fakeInputs[messHist.length].length) return;
          ...
          <div className="p-4 bg-white border-t">
            <form onSubmit={handleSend} className="relative">
                <input
                    ...
                    value={input}
                    onChange={(e) => {
                        const result = fakeInputs[messHist.length].slice(0, e.target.value.length);
                        setInput(result);
                    }}
