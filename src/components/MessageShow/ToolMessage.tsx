...
export default function ToolMessage({ message }: ToolMessage) {
    switch (message.content.type) {
        case "image":
            ...
            return (
                <div className={`flex ${message.content.owner === "user" ? "justify-end" : "justify-start"}`}>
                    {isGen ? (
        ...
        case "catalog":
            return (
                <div className="border px-5 py-2 rounded">
                    <p className="text-medium font-semibold">{message.content.title} - {message.content.quant}</p>
